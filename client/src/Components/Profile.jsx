...
function Profile() {
  ...
  const mutation = useMutation(
    (userId) => {
      return Axios.put(`/user/follow-unfollow/${userId}`, {
  ...
  useEffect(() => {
    if (listModal) {
      try {
        if (list === "Followers") {
          Axios.get(`/user/followers/${profileUser._id}`)
            .then(({ data }) => setListData(data))
            .catch((error) => errorHandler());
        } else if (list === "Following") {
          Axios.get(`/user/followings/${profileUser._id}`)
            .then(({ data }) => setListData(data))
            .catch((error) => errorHandler());
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (Object.keys(formErrors).length === 0 && isSubmit) {
      try {
        Axios.put(`/user/update/${currentUser._id}`, formValues)
          .then((response) => {
            updateCurrentUser();
          })
          .catch(({ response }) => {
  ...
  const handleCoverUpload = (e) => {
    ...
    try {
      Axios.put(
        `/user/cover-update/${currentUser._id}`,
        formdata,
        pictureConfig
      )
        .then((response) => {
  ...
  const handleProfileUpload = (e) => {
    ...
    try {
      Axios.put(
        `/user/profile-update/${currentUser._id}`,
        formdata,
        pictureConfig
      )
        .then((response) => {
  ...
  return (
    <>
                ...
                <div className=" relative">
                  ...
                  {currentUser?._id === profileUser?._id && (
                    <div
                      ...
                      className="z-0 absolute right-3 bottom-6 shadow bg-white rounded-full p-1"
                    >
                      <EditIcon />
                    </div>
                  )}
                  ...
                  {currentUser?._id === profileUser?._id && (
                    <EditIcon
                      ...
                      className="z-0 absolute left-32 bottom-11 shadow bg-white rounded-full p-1"
                    />
                  )}
                </div>
                <div className="bg-purple-300  -top-5 mb-6 h-auto flex flex-wrap content-center justify-between items-center relative rounded-3xl ">
                  <div className="flex flex-col  justify-center py-8 px-10 ">
                    ...
                    <div className="flex flex-wrap justify-center">
                      {currentUser?._id === profileUser?._id ? (
                        <button
                          className=" border border-black text-md font-semibold p-1 mt-2 rounded-md"
                          onClick={() => {
                            setUpdateModal(true);
                          }}
                        >
                        ...
                        <button
                          ...
                          className="m-1 focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 "
                        >
                          Following
                        </button>
                      ) : (
                        <button
                          onClick={(e) => handleFollow(e, profileUser._id)}
                          className="m-1 focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 "
                        >
                          Follow
                        </button>
      ...
      {currentUser?._id === profileUser?._id && (
        <Share profileUpdate={getUserPosts} />
      )}
      {profilePosts.length > 0 ? (
        profilePosts.map((post) => (
          <Post post={post} key={post._id} profileUpdate={getUserPosts} />
        ))
      ) : (
        <div className="flex flex-col justify-center items-center">
          <span className="text-xl text-gray-400">There are no posts yet.</span>
        </div>
      )}
