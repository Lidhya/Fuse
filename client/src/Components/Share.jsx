...
export default function Share({ profileUpdate }) {
  ...
  const mutation = useMutation(
    (formData) => {
      return Axios.post(`/post/create-post/${id}`, formData, pictureConfig)
        .then((response) => {
          setLoading(false);
  ...
  const handleSubmit = () => {
    if (!image && !video && !description)
      return setErrorMessage("There is nothing to post");
              ...
              <div className="flex flex-wrap ">
                <div className="flex items-center mx-2 cursor-pointer">
                  <label htmlFor="image-upload">
                    <PermMediaIcon htmlColor="gray" className="text-lg mr-1" />
                  </label>
                  <input
                    name="image"
                    className="hidden"
                    onChange={(e) => {
                      setErrorMessage("");
                      if (e.target.files[0].size > uploadMax)
          ...
          <div className="flex flex-wrap items-center justify-between">
            <div className="flex flex-wrap m-3">
              ...
              >
                ...
                <span className="md:text-md sm:text-sm font-semibold">
                  Photo
                </span>
              </div>
              <div
